...
class SimplePDO {
    ...
    public function __construct(array $classDB) {

        $this->port = (isset($classDB['port'])) ? "port=".$classDB['port'].";":"";

        if(!empty($classDB['driver'])){
            try {

                if($classDB['driver'] == "sqlite"){
                    $this->dbh = new SafePDO( $classDB['driver'].":".$classDB['path'] );
                }elseif($classDB['driver'] == "mysql"){
                    $this->dbh = new SafePDO( 'mysql:host='.$classDB['host'].';dbname='.$classDB['dbname'].'', $classDB['user'], $classDB['password'] );
                }else{
                    $this->dbh = new SafePDO( ''.$classDB['driver'].':host='.$classDB['host'].$this->port.';dbname='.$classDB['dbname'].'', $classDB['user'], $classDB['password']  );
                }
