 ...
 *
 * Класс модели основа работч с базой данных.
 * Класс имеет алисы от класса родителя, для удобства.
 */

namespace Core;


class Model extends \Library\SimplePDO
{
    ...
    public function connect($dbNameConnect)
    {
        if (array_key_exists($dbNameConnect, $this->dbConfConSett) AND !in_array($dbNameConnect, $this->dbConfConName))
        {
            ...
            if (class_exists('SimplePDO'))
                return $this->$dbNameConnect = new \Library\SimplePDO($dbConfigConnects);
